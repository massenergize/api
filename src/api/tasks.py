...
def download_data(self, args, download_type):
    store = DownloadStore()
    context = Context()
    context.user_is_community_admin = args.get(
        "user_is_community_admin", False)
    ...
    if download_type == "users":
        (files, com_name), err = store.users_download(
            context, community_id=args.get("community_id"), team_id=args.get("team_id"))
        if err is None:
            generate_csv_and_email(data=files, download_type='users',
                              community_name=com_name, email=email)

    elif download_type == 'actions':
        (files, com_name), err = store.actions_download(
            context, community_id=args.get("community_id"))
        if err is None:
            generate_csv_and_email(data=files, download_type='actions',
                              community_name=com_name, email=email)
        ...
        (files, com_name), err = store.teams_download(
            context, community_id=args.get("community_id"))
        if err is None:
            generate_csv_and_email(data=files, download_type='teams',
                              community_name=com_name, email=email)

    elif download_type == 'metrics':
        (files, com_name), err = store.metrics_download(
            context, args, community_id=args.get("community_id"))
        if err is None:
            generate_csv_and_email(data=files, download_type='metrics',
                              community_name=com_name, email=email)
