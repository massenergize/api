...
class CommunitiesTestCase(TestCase):
  ...
  def test_update_community_notification_settings(self):
    ...
    # signinAs(self.client, None)
    # update_response = self.client.post('/api/communities.notifications.settings.update', urlencode(args), content_type="application/x-www-form-urlencoded")
    # print("==RES==>", update_response)
    # update_response = update_response.toDict()
    # self.assertFalse(update_response["success"])
