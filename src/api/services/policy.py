...
class PolicyService:
  ...
  def create_policy(self,context, community_id, args) -> Tuple[dict, MassEnergizeAPIError]:
    policy, err = self.store.create_policy(context,community_id, args)
    if err:
      return None, err
    return serialize(policy), None

  def update_policy(self, context, policy_id, args) -> Tuple[dict, MassEnergizeAPIError]:
    policy, err = self.store.update_policy(context,policy_id, args)
    if err:
      return None, err
    return serialize(policy), None
  
  def delete_policy(self, context,policy_id) -> Tuple[dict, MassEnergizeAPIError]:
    policy, err = self.store.delete_policy(context,policy_id)
    if err:
      return None, err
    return serialize(policy), None
