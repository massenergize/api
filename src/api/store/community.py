...
class CommunityStore:
  ...
  def list_communities(self, context: Context, args) -> (list, MassEnergizeAPIError):
    try:
      if context.is_dev:
        communities = Community.objects.select_related('logo').filter(is_deleted=False, is_approved=True).exclude(subdomain='template')
      else:
        communities = Community.objects.select_related('logo').filter(is_deleted=False, is_approved=True, is_published=True).exclude(subdomain='template')
