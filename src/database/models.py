...
class FeatureFlag(models.Model):
    ...
    scope = models.CharField(max_length=SHORT_STR_LEN,default=FeatureFlagCostants.forUserFrontend()) # Is Backend/AdminFrontend/Portal Frontend etc.
    audience = models.CharField(max_length=SHORT_STR_LEN, default=FeatureFlagCostants.forEveryone()) # Community Audience: For Everyone/ Specific Communities / Or Everyone Except
    user_audience = models.CharField(max_length=SHORT_STR_LEN, default=FeatureFlagCostants.forEveryone()) # User Audience: For Everyone/ Specific Communities / Or Everyone Except
    key = models.CharField(max_length=SHORT_STR_LEN, unique=True) # Special key that makes sense, and we can use for easy look-up Eg. (guest_authentication_feature)
    # is_frontend = models.BooleanField(default=False)
    # is_backend = models.BooleanField(default=False)
    communities = models.ManyToManyField(Community, blank=True, related_name="community_features")
    # excluded_communities =  models.ManyToManyField(Community, blank=True, related_name="deactivated_features")
