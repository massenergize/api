...
def remove_duplicate_images(task : Task = None):
    ...
    clean_and_notify(search_context,com_names,task.creator if task else None, do_deletion)
...
def clean_and_notify(ids,community_names,notification_receiver, do_deletion): 
        ...
        send_summary_email_to_admin(notification_receiver, community_names, num_of_dupes_in_all, csv_file)


def send_summary_email_to_admin(admin, community_names, total, csv_file, **kwargs):
    memory_saved = kwargs.get("memory_saved",None)
    args = {
