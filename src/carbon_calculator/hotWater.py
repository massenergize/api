...
def EvalHotWaterAssessment(inputs):
    ...
    if inputs.get('hot_water_assessment', YES) == YES:
        ...
        gallons_per_person = getDefault(locality,'water_hw_use_per_person')
        ...
        wh_set_temp = getDefault(locality,'water_heater_settemp') 
        wh_input_water_temp = getDefault(locality,'water_input_temp')
        ...
        if wh_type == "Electric":
            ...
            wh_efficiency = getDefault(locality,'water_heater_electric_efficiency')
            btu = daily_hw_use * water_specific_heat * water_deltaT * 365/ wh_efficiency
        ...
        conversion_rate = getDefault('locality', 'water_hw_assessment_conversion_rate')
...
def EvalHeatPumpWaterHeater(inputs):
    ...
    if inputs.get('replace_water_heater', YES) == YES:
        ...
        cost = getDefault(locality,'water_hpwh_installed_price')
        rebate = getDefault(locality, 'water_hpwh_rebate')
        cost = cost - rebate

        decent_payback = getDefault(locality,'general_decent_home_investment_payback')
...
def EvalSolarHW(inputs):
    ...
    if inputs.get('install_solar_hw', YES) == YES:
        ...
        if potential<0.5:
            ...
            solar_fraction = getDefault(locality,'solar_wh_system_fraction')
            ...
            state_rebate = getDefault(locality,'solar_hw_state_rebate')
            utility_rebate = getDefault(locality,'solar_hw_utility_rebate')
            
            points = solar_fraction * co2_old
            savings = solar_fraction * cost_old

            # too simplistic I think
            system_cost = getDefault(locality,'solar_hw_system_average_cost') / potential
