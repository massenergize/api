...
def EvalSolarAssessment(inputs):
    ...
    if inputs.get('solar_assessment',YES) == YES:
        ...
        default_size = getDefault(locality,'solar_default_pv_size')
        ...
        if spotential != "None" and spotential != "Poor":
            ...
            annual_kwh_per_kw = getDefault(locality,'solar_annual_kwh_per_kw')
            annual_kwh = annual_kwh_per_kw * size * potential

            assessment_conversion = getDefault(locality,'solar_assessment_convertion_fraction')
...
def EvalSolarPV(inputs):
    ...
    if inputs.get('install_solar_panels',YES) == YES:
        ...
        incentive_payment_per_kwh = 0.001 * getDefault(locality,'solar_srec_payment_per_mwh')
        local_price_factor = getDefault(locality,'solar_local_price_factor')
        tax_credit = getDefault(locality,'solar_federal_tax_credit')   # for 2020
        state_credit = getDefault(locality,'solar_state_tax_credit')
        state_rebate = getDefault(locality,'solar_state_rebate')
        utility_rebate = getDefault(locality,'solar_utility_rebate')
        array_cost_per_kw = getDefault(locality,'solar_pv_kw_cost')
